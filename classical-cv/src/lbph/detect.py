...
def validate_loaded_lbph_model(model_path: str, recognizer, descriptor_metadata: dict) -> None:
    """Validate YAML parameters after OpenCV read restored the model state."""
    expected = {key: descriptor_metadata[key] for key in ("radius", "neighbors", "grid_x", "grid_y")}
    storage = cv.FileStorage(model_path, cv.FILE_STORAGE_READ)
    root = storage.getNode("opencv_lbphfaces")
    actual = {
    ...
    if actual != expected or restored != expected:
        raise RuntimeError(
            f"LBPH model/config mismatch for {model_path}: "
            f"yaml={actual}, restored={restored}, expected={expected} "
            f"({descriptor_metadata['id']})"
        )
...
def main() -> None:
    ...
    validate_loaded_lbph_model(args.model_path, recognizer, descriptor_metadata)
