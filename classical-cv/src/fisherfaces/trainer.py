"""Thin Fisherfaces training wrapper over the shared classical pipeline.

All real work (preprocessing, dataset gathering, training, footprint) lives in
:mod:`src.classical_faces.pipeline`. This file only declares Fisherfaces-specific
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train a Fisherfaces recognizer on the clean La Salle split by default."
    )
    add_dataset_args(
        parser,
    ...
    parser.add_argument(
        ...
        help="Retained LDA components (0 = classes - 1).",
    )
