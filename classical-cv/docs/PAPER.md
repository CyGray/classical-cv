...
> **Source synchronization note (2026-08-13):** The Springer DOCM is the submission artifact. Corrected version 012 is derived directly from exact version 011 and preserves its one-figure layout while adding the bounded gate accept-protection result. The canonical replay is descriptive and post-hoc, the runtime gate is unchanged, and target-device testing is outside scope.
...
| La Salle DB2 / LSDB-DL41 (held-out 41-modification probes) | paired complementarity and gate evaluation | target-domain error recovery, paired errors, McNemar evidence, and routing; not the headline robustness benchmark |
| LFW DB1 (5,749 ids, clean) | independence sweep | out-of-domain transfer with real statistical power |
| LFW DB2 / LFW2-DL41 (41 modifications; gallery/probe-disjoint) | 1-to-N identification robustness (Accuracy Ratio) | out-of-domain degradation and per-modification robustness at frozen thresholds |
...
The canonical LSDB-DL41 rerun measures gate competence on 2,060 scored modified probes: LBPH distance predicts threshold-free LBPH Rank-1 error with **ROC AUC 0.950**, and negative relative margin reaches **0.953**. Under the deployed policy, the cascade matches SFace-only acceptance at **87.24%**, but escalates **71.52%** of the 2,296 transformed probes and has a stored recognition-stage arithmetic mean of **11.96 ms**, versus **8.33 ms** for direct SFace.

A canonical descriptive-only accept-protection replay then asks whether quality flags should override an LBPH decision already inside `tau_accept`. Keeping the deployed `margin < 0.05` trigger and all routing above `tau_accept` unchanged, but treating accept-side quality flags as telemetry, preserves **87.24% AR** on the same probes while reducing escalation to **59.23%**. It reduces escalations of thresholded LBPH-correct probes from **289/707** to **7/707** and the stored arithmetic mean to **10.81 ms**. Direct SFace remains faster. Because the candidate was selected and evaluated on the same 56 known-genuine source images and correlated transforms, this is a post-hoc routing ablation, not independent validation or a deployment-policy claim; the runtime gate remains unchanged.
...
Four lessons. First, closed-set accuracy misleads: rank-1 orders the classical methods 100/75/66, but at fixed FAR, the gate's real operating mode, only LBPH survives. Second, thresholds should come from impostor distributions, not validation splits: the k-th-error-pair rule ties the operating point to an exact error count, reproduces both spec anchors, and exposed that La Salle resolves only ~1,300 ppm, which defers the 100 ppm claim to LFW. Those distributions do not transfer intact (LBPH's compresses 15.9 points from La Salle to LFW), so the safe move is to freeze on the hardest database, and the measured cascade FAR (about 0.01% on every LFW leg against 1.39% on the tightly-matched La Salle cohort) confirms that anchoring on the harder set makes the transfer conservative rather than loose. Third, complementarity is a measurement, and measuring it split the claim in two: it holds on the accuracy axis and fails on the impostor axis. Fourth, a cascade does not need independent errors, only a competent gate and a conjunction. The descriptive accept-protection replay narrows redundant routing without changing AR, but its reconstructed **10.81 ms** mean remains slower than direct SFace and it does not change the deployed gate. This paper confines that observation to the repository's known-genuine replay; open-set identification and target-device timing are outside scope.

## 5. Conclusion

LS-Face selects and fuses its recognizers through independence testing. The exhaustive impostor sweep gave every engine a threshold at a specified FAR and established the complementarity boundary: SFace recovers many LBPH misses, while the cascade's route cost depends on how often uncertain probes reach SFace. The canonical descriptive accept-protection replay identifies a simpler candidate rule within that boundary. Treating accept-side quality flags as telemetry while retaining the deployed low-margin trigger reduces LSDB-DL41 escalation from **71.52%** to **59.23%** without changing **87.24% AR**, and reduces the stored arithmetic mean from **11.96** to **10.81 ms**. It remains slower than direct SFace, was selected and evaluated on the same known-genuine transformed probes, and does not modify the runtime gate. The result is therefore a bounded routing observation, not an independently validated deployment claim; open-set and target-device measurements are outside the present paper's scope.
