...
Usage::
    ...
    python scripts/pipeline/run_classical_clean_loop.py                 # full loop
    python scripts/pipeline/run_classical_clean_loop.py --regenerate-aug
    python scripts/pipeline/run_classical_clean_loop.py --skip-aug --families lbph
"""
...
def regenerate_aug(py: str) -> None:
    run(
        [
            py,
            "scripts/utils/augment_split_light_medium.py",
            "--split-root",
            SPLIT_BASE,
            "--output-root",
            AUG_ROOT,
...
def evaluate(py: str, family: str, variant: str) -> None:
    ...
    suffix = "" if variant == "baseline" else "_aug_light"
    report = f"outputs/evaluation/{family}_eval{suffix}.json"
    run(
        [
