...
Usage::
    ...
    python scripts/pipeline/run_independence_thresholds.py
    python scripts/pipeline/run_independence_thresholds.py --skip-run        # consolidate only
    python scripts/pipeline/run_independence_thresholds.py --error-pair-rank 8
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-json", default="outputs/benchmark/independence_thresholds.json")
...
def load_spec(out_dir: str) -> dict | None:
    family = Path(out_dir).name.replace("_lasalle", "").replace("_lfw", "")
    candidates = [
        PROJECT_ROOT / out_dir / "summary.json",
        PROJECT_ROOT / "outputs" / "independence" / family / f"{family}_lasalle" / "summary.json",
        PROJECT_ROOT / "outputs" / "independence" / family / f"{family}_lasalle_native" / "summary.json",
        PROJECT_ROOT / "outputs" / "independence" / family / "old" / f"{family}_lasalle" / "summary.json",
    ]
    if family == "lbph":
        candidates.append(PROJECT_ROOT / "outputs" / "independence" / "hybrid" / "lsdb1_fixed" / "summary.json")
    for path in candidates:
        if path.exists():
            try:
                with path.open("r", encoding="utf-8") as f:
                    summary = json.load(f)
                report = summary.get("error_pair_thresholds") or summary.get("lbph_rank_thresholds")
                if isinstance(report, dict) and report.get("spec"):
                    return report.get("spec")
            except Exception:
                continue
    return None
...
def main() -> None:
    ...
    for family in families:
        out_dir = f"outputs/independence/{family}_lasalle"
