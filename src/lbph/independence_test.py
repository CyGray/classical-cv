"""
LBPH Independence Test: Verify distance distribution and false positive behavior
without comparing images from the same identity.

This test:
1. Selects one image per person from the dataset
2. Trains LBPH model on these selected images
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="LBPH Independence Test: N×(N-1) cross-identity distance analysis."
    )
    parser.add_argument(
        "--dataset-dir",
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Optional LBPH distance threshold (normalized 0-100 scale) for false positive analysis.",
    )
    parser.add_argument(
        "--random-seed",
        type=int,
    ...
    parser.add_argument(
        ...
        default=SPECS["lbph"].default_equalization,
...
def compute_pairwise_distances(
    ...
    histograms from the saved model, and compute N×(N-1) pairwise
    chi-squared distances.  All raw distances are then normalised to the
    [0, 100] scale so results are comparable across models.
    """
    ...
    records_data = common_pairwise_distances(
        feature_matrix, identities_sorted, model_type="lbph",
    )
...
def save_csv_results(
    ...
    """Save all comparison records to CSV with raw and normalized distance."""
...
def print_report(
    ...
    print("\n" + "="*80)
    print(f"{model_label} INDEPENDENCE TEST REPORT")
    print("="*80)
...
def run_lbph_independence_test() -> int:
...
if __name__ == "__main__":
    raise SystemExit(run_lbph_independence_test())
