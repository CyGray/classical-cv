"""Thin LBPH evaluation wrapper over the shared classical pipeline.

The evaluation loop, metrics, threshold sweep, and report schema live in
:mod:`src.classical_faces.pipeline`. Default loop evaluates the clean held-out
La Salle test split (``data/split_lasalle/test``) against the clean model, with
``--assume-cropped`` so no Haar detection / full-image fallback occurs.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate an LBPH model on the clean held-out La Salle test split by default."
    )
    add_dataset_args(
        parser,
