"""Thin Eigenfaces training wrapper over the shared classical pipeline.

All real work (preprocessing, dataset gathering, training, footprint) lives in
:mod:`src.classical_faces.pipeline`. This file only declares Eigenfaces-specific
defaults. Default loop is leakage-free: train on the pre-cropped held-out La Salle
split (``data/split_lasalle/train``) with ``--assume-cropped``.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train an Eigenfaces recognizer on the clean La Salle split by default."
    )
    add_dataset_args(
        parser,
    ...
    parser.add_argument(
        "--num-components",
        type=int,
        default=SPECS[FAMILY].default_num_components,
        help="Retained PCA components (0 = all training samples).",
    )
