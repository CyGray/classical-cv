"""
Train Eigenfaces using exactly one image per person: light front.png.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train Eigenfaces using one image per identity: light front.png."
    )
    parser.add_argument(
        "--dataset-dir",
    ...
    parser.add_argument(
        "--model-output",
        default=root_path("models", "eigenfaces", "trainer_light_front.yml"),
        help="Output path for trained Eigenfaces model.",
    )
    parser.add_argument(
        "--labels-output",
        default=root_path("models", "eigenfaces", "labels_light_front.json"),
...
def main() -> None:
    ...
    print("[INFO] Eigenfaces Light-Front Training")
    ...
    num_components = min(len(faces) - 1, 50)
