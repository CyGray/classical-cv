"""
Eigenfaces Independence Test using only light_front.jpg for each identity.
...
DEFAULT_OUTPUT_DIR = str(PROJECT_ROOT.joinpath("outputs", "eigenfaces", "independence_test_light_front"))
ALLOWED_EXTENSIONS = {".png", ".jpg", ".jpeg", ".bmp", ".webp"}
_PAIRWISE_FEATURE_PATH = ""
_PAIRWISE_METRIC = "eigenfaces"
...
def load_or_build_feature_cache(
    ...
    *,
    ...
    cache_enabled = args.dataset_source == "lfw-dataset"
    if not cache_enabled:
        return train_and_extract_features(faces, label_map, model_type=model_type)

    cache_dir = Path(PROJECT_ROOT) / "outputs" / model_type / "_cache"
    cache_dir.mkdir(parents=True, exist_ok=True)

    identity_keys = sorted(label_map.keys())
    key_parts = [
        model_type,
        args.dataset_source,
        args.dataset_dir,
        args.image_name,
        str(args.equalization),
        str(bool(args.align_eyes)),
        str(args.min_face_size),
        str(args.downscale_max_side),
        str(len(identity_keys)),
    ]
    sig = "|".join(key_parts + identity_keys)
    import hashlib
    cache_id = hashlib.sha1(sig.encode("utf-8")).hexdigest()[:16]
    npy_path = cache_dir / f"features_{cache_id}.npy"
    ids_path = cache_dir / f"identities_{cache_id}.json"

    if npy_path.exists() and ids_path.exists():
        try:
            features = np.load(str(npy_path), mmap_mode=None)
            with ids_path.open("r", encoding="utf-8") as f:
                identities = json.load(f)
            if isinstance(identities, list) and len(identities) == features.shape[0]:
                print(f"[CACHE] Reusing cached {model_type} features: {npy_path}")
                return np.asarray(features, dtype=np.float64), [str(x) for x in identities]
        except Exception:
            pass

    features, identities = train_and_extract_features(faces, label_map, model_type=model_type)
    try:
        np.save(str(npy_path), np.asarray(features, dtype=np.float64))
        with ids_path.open("w", encoding="utf-8") as f:
            json.dump(list(identities), f, indent=2)
        print(f"[CACHE] Saved {model_type} features: {npy_path}")
    except Exception:
        pass
...
def _faces_cache_paths(args: argparse.Namespace, model_type: str) -> tuple[Path, Path]:
    import hashlib
    cache_dir = Path(PROJECT_ROOT) / "outputs" / model_type / "_cache"
    cache_dir.mkdir(parents=True, exist_ok=True)
    sig = "|".join([
    ...
    return cache_dir / f"faces_{key}.npz", cache_dir / f"labels_{key}.json"

def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Eigenfaces Independence Test using only light_front.jpg."
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Optional Eigenfaces distance threshold on the normalized 0-100 scale.",
    )
    parser.add_argument(
        "--min-face-size",
        type=int,
...
def resolve_default_output_dir(args: argparse.Namespace) -> None:
    ...
    args.output_dir = root_path("outputs", "eigenfaces", "independence_test_light_front", suffix)
...
def compute_pairwise_distances(
    ...
    # Actually for Eigenfaces it should be Euclidean.
...
def save_distance_histogram(
    ...
    plt.title("Eigenfaces Independence Test: Inter-Identity Distance Histogram")
...
def save_distance_curve_plot(
    ...
    plt.title("Eigenfaces Independence Test: Inter-Identity Distance Curve")
    plt.xlabel("Normalized Distance")
...
def print_report(summary: Dict) -> None:
    print("\nEIGENFACES LIGHT-FRONT INDEPENDENCE TEST REPORT (AGGREGATED)")
...
def run_eigenfaces_independence_test_light_front() -> int:
    args = parse_args()
...
if __name__ == "__main__":
    raise SystemExit(run_eigenfaces_independence_test_light_front())
