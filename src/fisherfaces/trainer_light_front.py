"""
Train Fisherfaces using exactly one image per person: light front.png.
"""
...
from ..lbph.preprocess import IMG_SIZE, extract_lbph_face, resolve_eye_cascade_path
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train Fisherfaces using one image per identity: light front.png."
    )
    parser.add_argument(
        "--dataset-dir",
    ...
    parser.add_argument(
        "--model-output",
        default=root_path("models", "fisherfaces", "trainer_light_front.yml"),
        help="Output path for trained Fisherfaces model.",
    )
    parser.add_argument(
        "--labels-output",
        default=root_path("models", "fisherfaces", "labels_light_front.json"),
...
def main() -> None:
    ...
    print("[INFO] Fisherfaces Light-Front Training")
    ...
    num_components = max(1, min(len(faces) - 2, 49))
    recognizer = cv.face.FisherFaceRecognizer_create(
        num_components=num_components, threshold=1e9,
    )
