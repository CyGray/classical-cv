...
FISHER_DISTANCE_LABEL = "Fisherfaces euclidean distance"
...
def run_fisherfaces_independence_test() -> int:
    ...
    if (n_est * (n_est - 1) // 2) > args.streaming_threshold:
        return _run_fisher_streaming(args, person_dirs, face_cascade, eye_cascade)
    ...
    if args.plots:
        summary["plots"] = write_default_plots(
            ...
            args.output_dir,
            model_label="Fisherfaces",
            distance_label=FISHER_DISTANCE_LABEL,
...
def _run_fisher_streaming(args, person_dirs, face_cascade, eye_cascade) -> int:
    ...
    selected_images = select_images_per_person(
        person_dirs, args.random_seed, args.images_per_identity
    )
    ...
    if len(faces) < 2:
        ...
        return 1
    print(f"[INFO] Extracting Fisherfaces projections for {len(faces)} identities ...")
    feature_matrix, names = train_and_extract_features(faces, label_map, model_type="fisherfaces")
    pre_stats = {
    ...
    return run_streaming_and_save(
        ...
        model_label="Fisherfaces", distance_label=FISHER_DISTANCE_LABEL,
        person_dirs=person_dirs, pre_stats=pre_stats, model_key="fisherfaces",
    )
