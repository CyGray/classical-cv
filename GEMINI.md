...
> **Current evidence addendum (2026-08-09):** `docs/results/` is the sole
> canonical manuscript-result store. Read the relevant category README,
> `RESULTS_MANIFEST.json`, and `metadata/PROVENANCE_AUDIT.md` before citing an
> artifact. `classical-cv/reports/` and `classical-cv/outputs/` are raw/working
> locations. Robustness tables stay under `docs/results/robustness_test/`, never
> complementarity. Do not cite stand-ins or provenance/rerun-flagged results.
> Use native Word tables; regenerate verified figures as SVG/PDF (or EMF on
> Windows), never by auto-tracing PNG rasters. This addendum supersedes any
> conflicting result-location, manuscript-figure, or threshold prose below;
> runtime threshold truth remains `classical-cv/src/hybrid/thresholds.json`.
>
> **Algorithm-selection addendum (2026-08-10):** Use
> `docs/results/algo_test/lsdb_selection_2026-08-10/` for the paper's first
> algorithm-selection result. It is the deterministic LSDB campaign (224 fit,
> 56 calibration, and 56 held-out test images): LBPH is the selected classical
> model; SFace is the external-team learned candidate selected for its
> deployment-compatible footprint, not a same-harness accuracy claim. This
> LSDB selection question is separate from frozen LFW deployment-threshold
> calibration. Do not transfer its temporary operating point into
> `thresholds.json` or describe it as an LFW result.
>
> **Manuscript visual-QA addendum (2026-08-10):** A DOCM edit is not complete
> until it is exported to PDF and the pages containing changed tables/figures
> are visually inspected. Check captions stay with their objects, table cells
> contain no inherited list numbers, column wrapping is readable, and the
> LNCS three-rule table design is intact.
>
> **Repository correction (2026-08-10):** The workspace root is one Git
> repository; run Git commands from this root. The older contrary statement in
> Sec. 0 is historical.
...
For every edit to `docs/manuscript/lsface.docm`, archive the before copy and
final after copy in `docs/manuscript/versions/`, following its exact sequential
`[index]_lsface_[version_name].docm` convention; do not use timestamps or
suffix-index names.
