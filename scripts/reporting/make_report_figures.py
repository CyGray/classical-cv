...
Usage:
    python scripts/reporting/make_report_figures.py                 # full LFW impostors (slow once)
    python scripts/reporting/make_report_figures.py --impostor-max-identities 300   # quick validate
    python scripts/reporting/make_report_figures.py --skip-roc      # tables + figs 2-4 only
"""
...
def load_json(path: Path) -> dict:
    ...
    if not p.exists():
        sp = str(p)
        if "reports" in sp:
            alt = Path(sp.replace("reports", "outputs"))
            if alt.exists():
                p = alt
        elif "outputs" in sp:
            alt = Path(sp.replace("outputs", "reports"))
            if alt.exists():
                p = alt
...
def build_tables(tar_far, accuracy, live, evals, indep_ls, indep_lfw) -> str:
    ...
    L += [
        ...
        "",
        "*Generated by `scripts/reporting/make_report_figures.py` from committed run artifacts "
...
def fig_histogram(indep_ls, summaries_ls):
    ...
    for ax, fam in zip(axes, FAMILIES):
        csv_candidates = [
            OUTPUTS / "independence" / fam / f"{fam}_lasalle" / "comparisons.csv",
            OUTPUTS / "independence" / f"{fam}_lasalle" / "comparisons.csv",
            OUTPUTS / "independence" / f"{fam}_lasalle_native" / "comparisons.csv",
            OUTPUTS / "independence" / fam / "old" / f"{fam}_lasalle" / "comparisons.csv",
            OUTPUTS / "independence" / "hybrid" / "lsdb1_fixed" / "comparisons.csv",
            REPORTS / "independence" / f"{fam}_lasalle" / "comparisons.csv",
        ]
        chosen_csv = next((c for c in csv_candidates if c.exists()), csv_candidates[0])
        dists = load_csv_distances(chosen_csv)
        spec = ls_rows[fam]
...
def main():
    ...
    OUTPUTS = PROJECT_ROOT / "outputs"
    tar_far = load_json(OUTPUTS / "benchmark" / "tar_at_far.json")
    accuracy = load_json(OUTPUTS / "benchmark" / "accuracy_ratio.json")
    live = load_json(OUTPUTS / "benchmark" / "live_fps" / "aggregate_summary.json")
    indep_ls = load_json(OUTPUTS / "benchmark" / "independence_thresholds.json")
    indep_lfw = load_json(OUTPUTS / "benchmark" / "independence_thresholds_lfw.json")
    evals = {f: load_json(OUTPUTS / "evaluation" / f"{f}_eval.json") for f in FAMILIES}
    summaries_ls = {}
    for r in indep_ls["rows"]:
        fam = r["family"]
        candidates = [
            OUTPUTS / "independence" / fam / f"{fam}_lasalle" / "summary.json",
            OUTPUTS / "independence" / fam / f"{fam}_lasalle_native" / "summary.json",
            OUTPUTS / "independence" / fam / "old" / f"{fam}_lasalle" / "summary.json",
            OUTPUTS / "independence" / "hybrid" / "lsdb1_fixed" / "summary.json",
            OUTPUTS / fam / "independence_test" / "summary.json",
        ]
        chosen = None
        for c in candidates:
            if c.exists():
                try:
                    summaries_ls[fam] = load_json(c)
                    chosen = c
                    break
                except Exception:
                    continue
        if chosen is None:
            summaries_ls[fam] = load_json(candidates[0])
