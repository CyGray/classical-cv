...
def main():
    ...
    parser.add_argument("--input-json", default=str(BENCHMARK_OUTPUTS_DIR / "accuracy_ratio_hybrid.json"),
    ...
    if not input_path.exists() and "outputs" in str(input_path):
        alt_path = Path(str(input_path).replace("outputs", "reports"))
        if alt_path.exists():
            input_path = alt_path
