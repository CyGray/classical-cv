...
Usage:
    python scripts/utils/md_to_pdf.py docs/PAPER_WRITING_GUIDE.md docs/PAPER_WRITING_GUIDE.pdf
"""
...
def parse(md: str, st) -> list:
    ...
    while i < len(lines):
        ...
        if m_img:
            flush()
            caption, img_rel = m_img.group(1), m_img.group(2)
            ...
            if img_path.exists():
                try:
                    ...
                    img_flow = Image(str(img_path), width=rw, height=rh)
                    cap_style = ParagraphStyle(
                        "caption", st["quote"], alignment=1, fontSize=8.2, leading=10.5,
                        textColor=MUTED, spaceBefore=4, spaceAfter=8
                    )
                    elems = [Spacer(1, 4), img_flow]
                    if caption:
                        elems.append(Paragraph(f"<i>Figure: {inline(caption)}</i>", cap_style))
                    elems.append(Spacer(1, 8))
                    flow.append(KeepTogether(elems))
                except Exception as err:
                    print(f"Warning: failed to load image {img_rel}: {err}")
            i += 1
            continue
        ...
        if stripped.startswith("```"):
            ...
            code_style = ParagraphStyle(
                "codeblock", st["body"], fontName="Courier", fontSize=7.6, leading=9.5,
                textColor=INK, backColor=BAND, borderPadding=5, spaceBefore=4, spaceAfter=8,
