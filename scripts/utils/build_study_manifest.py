...
Pipeline (docs/ui/DESIGN.md §7):
    ...
    docs/ui/study_status.yml  ->  scripts/utils/build_study_manifest.py  ->
...
Run:  python scripts/utils/build_study_manifest.py
...
BENCHMARK_SOURCES = [
    "outputs/benchmark/accuracy_ratio_hybrid.json",
    "outputs/benchmark/gate_operating_curve.json",
    "outputs/benchmark/evidence_matrix.json",
]
...
def _copy_results() -> int:
    ...
    n = 0
    root = REPO_ROOT / "outputs" / "independence"
    if not root.exists() or not list(root.rglob("summary.json")):
        alt_root = REPO_ROOT / "reports" / "independence"
        if alt_root.exists():
            root = alt_root
