...
`scripts/make_selection_manifest.py`). Campaign orchestration runs through
`scripts/lfw2_worker.py`; merging through `scripts/merge_lfw2_segments.py`;
the 41-row table and figure through `scripts/build_systematic_matrix.py`.
...
Members do not run this directly — `scripts/lfw2_worker.py --member <name>`
...
(`scripts/lfw2_worker.py`, `DESIGN.md` §6.1, BUILT + dry-run verified): one
...
with `scripts/merge_lfw2_segments.py` (coverage proof + count-sum merge,
verified byte-identical reassembly), then builds the 41-row tier-grouped
`systematic_matrix.md`/`.json` + FAR figure with
`scripts/build_systematic_matrix.py` (missing variants render as PENDING,
