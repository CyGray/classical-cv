...
**Scripts:** `scripts/utils/setup_datasets.py` (download + link datasets), `scripts/utils/augment_split_light_medium.py` (generate augmented tiers).
...
Both are reported so nothing is oversold. *Script: `scripts/archive/per_run_thresholds.py` (backfills per-run thresholds from committed `_raw_runs/`), implemented by `per_run_error_pair_thresholds()` in `src/independence_common.py`.*
...
```bash
python scripts/pipeline/run_lfw_independence.py   # streamed top-K path (src/independence_streaming.py); helper: scripts/pipeline/run_independence_thresholds.py
```
...
```bash
python scripts/reporting/make_report_figures.py          # → reports/figures/*.png
python scripts/reporting/make_hybrid_report_figures.py   # → docs/figures/*.png
```
...
|---|---|---|
| Dataset setup / augmentation | `scripts/utils/setup_datasets.py`, `scripts/utils/augment_split_light_medium.py` | `data/…` |
...
| Full-LFW streamed sweep | `scripts/pipeline/run_lfw_independence.py` (uses `src/independence_streaming.py`, `src/independence_report.py`) | independence reports |
| Per-run threshold stability | `scripts/archive/per_run_thresholds.py` | console / JSON |
...
| Figures | `scripts/reporting/make_report_figures.py`, `scripts/reporting/make_hybrid_report_figures.py` | `reports/figures/`, `docs/figures/` |
